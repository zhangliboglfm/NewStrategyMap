...
<script type="text/javascript">
	...
	var tabwidtharr=[0.1,0.1,0.25,0.35,0.1,0.1];
	var tabcolumn=["indexBigType","indexSmallType","indexName","indexDesc","indexDateType"];
	var tabtitledata=["指标类型","指标细分","指标名称","指标口径","数据周期","操作"];
	...
	function queryIndex(){
		...
		$.ajax({
			...
			type:"post",
			dataType:"json",
			data:{"keystr":key,"querytype":querytype,dateType:dateType},
	...
	function mosaicSearchResultCode(data){
		...
		if(data.length!=0){
			...
			for(var i=0;i<data.length;i++){
				...
				htmlcode+="<td onclick='backindexid(this);'  index-unit='"+obj.indexunit+"'  index-id='"+obj.indexId+"'  indexname='"+obj.indexName+"' style='width: "+(ws*(tabwidth-boderwidth))+"px;background-color: "+tdbg+";color:#50d1f2;cursor:pointer'>添加</td>";
				tr.html(htmlcode);
				tbody.append(tr);
			}
			//加载滚动条
   ...
   //点击添加调用父类的方法返回给父类参数
     function backindexid(e){
         var indexid=$(e).attr("index-id");
         var indexname=$(e).attr("indexname");
         var indexunit=$(e).attr("index-unit");
         parent.addeachindex(indexid,indexname,indexunit);
     }
