...
<script type="text/javascript">
  ...
  function initKuandaiData(data){
  	if(data.length !=0){
  	    var barheight = $(".everyinternetdatabart").height();
        var barwidth=$(".everyinternetdatabart").width();
	  	for(var i=0;i<data.length;i++){
	  		var obj = data[i];
	  		$("#div"+obj.regionId+"Online").find(".kuandaiprodiv").find(".prodiv").velocity({
				width:obj.onlineData*barwidth+"px"
			},{
				duration:800
			});
...
/* 	function initFluxProgress(data){
  ...
  function adjust(){
      ...
      $("#divKuandaiProgressData").find(".everyinternetdata").each(function(i,e){
    	$(e).find(".everyinternetdatabart").append(initProgress(1, barwidth, barheight, "blue", "big"));
      });
