...
public class CommonInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if(userId == null || userId ==""){
			...
			TbLoginNumberZc obj =systemManager.tranUserLoginData(userId);
			String  loginIp ="";
			try {
			...
			//System.out.println("登录ip为"+loginIp);
			String ip =LogUtil.getIpAddr(request);  //获取客户端IP地址
			//System.out.println("本机ip为"+ip);
			if(!ip.equals(loginIp)){
				request.setAttribute("4A", "0");
			}else{
				request.setAttribute("4A", "1");
			}*/
		}else{
			//第一次请求数据，request中带有 userId
			TbLoginNumberZc obj =systemManager.tranUserLoginData(userId);
			String  loginIp ="";
			try {
				loginIp = obj.getLoginIp();
