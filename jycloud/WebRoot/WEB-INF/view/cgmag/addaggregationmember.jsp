...
<html>
  ...
  <link rel="stylesheet" href="layui/css/layui.css" type="text/css"></link><script type="text/javascript" src="../../../layui/lay/modules/laypage.js"></script><script type="text/javascript" src="js/aggregation.js"></script></head>
  <style type="text/css">
  </style>
  <body>
		 ...
		 <form class="layui-form" action="" lay-filter="example">
		  ...
		  <div class="layui-form-item">
		    <div id="btn1" class="layui-input-block">
		      <button class="layui-btn" onclick="reject()">提交</button>
		    </div>
		  </div>
		</form>
     </div> 
...
<script>
 var compid=window.parent.sondata;
...
function getagsearchdata(){
		var cgname=$("#name").val();
	     $.ajax({
			url:"searchaggregationagdata.do",
			type:"post",
			data:{"compid":compid,"cgId":cgId,"cgname":cgname},
			dataType:"json",
...
function getagdata(){
	     $.ajax({
			url:"getaggregationagdata.do",
			type:"post",
			data:{"compid":compid,"cgId":cgId},
			dataType:"json",
			success:function(data){
				for(var i=0;i<data.length;i++){
					createCheckbox(data[i]);
...
function createCheckbox(data){
	...
	function reject(){
			...
			var worksid=$(this).val();
 		 	$.ajax({
				url:"addaggregationmemberdata.do",
				type:"post",
				async: false,
				data:{"compid":compid,"worksname":worksname,"worksid":worksid},
				success:function(data){
					/* alert(data); */
				}
			});
