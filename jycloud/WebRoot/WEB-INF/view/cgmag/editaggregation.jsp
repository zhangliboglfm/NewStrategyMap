<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>
<%
String path = request.getContextPath();
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
%>

<!DOCTYPE HTML>
<html>
  <head>
     <base href="<%=basePath%>">
    <title>添加成就</title>
    
	<meta http-equiv="pragma" content="no-cache">
	<meta http-equiv="cache-control" content="no-cache">
	<meta http-equiv="expires" content="0">    
	<meta http-equiv="keywords" content="keyword1,keyword2,keyword3">
	<meta http-equiv="description" content="This is my page">
	<link rel="stylesheet" href="layui/css/layui.css" type="text/css"></link>
	<link rel="stylesheet" href="css/login.css" type="text/css"></link>
	<link rel="stylesheet" href="css/admin.css" type="text/css"></link>
	<script type="text/javascript" src="layui/lay/modules/element.js"></script>
	<script type="text/javascript" src="js/jquery-1.9.1.js"></script>
	<!--
	<link rel="stylesheet" type="text/css" href="styles.css">
	-->
<script type="text/javascript" src="../../../layui/lay/modules/laypage.js"></script></head>
  <style type="text/css">
	@font-face {font-family: "iconfont";
	  src: url('iconfont.eot?t=1533979646640'); /* IE9*/
	  src: url('iconfont.eot?t=1533979646640#iefix') format('embedded-opentype'), /* IE6-IE8 */
	  url('data:application/x-font-woff;charset=utf-8;base64,') format('woff'),
	  url('iconfont.ttf?t=1533979646640') format('truetype'), /* chrome, firefox, opera, Safari, Android, iOS 4.2+*/
	  url('iconfont.svg?t=1533979646640#iconfont') format('svg'); /* iOS 4.1- */
	}
	
	.iconfont {
	  font-family:"iconfont" !important;
	  font-size:16px;
	  font-style:normal;
	  -webkit-font-smoothing: antialiased;
	  -moz-osx-font-smoothing: grayscale;
	}  
.addDiv{
	height: 116px;
    width: 116px;
    float: left;
    margin: 2px;
    text-align: center;
    border: none;
    position: relative;
    z-index: 1;
}
#workarea{
	height:240px;
	width:700px;
	margin-left:110px;
	overflow-y:auto;
}
.addDiv .add{
    width: 84px;
    height: 84px;
    line-height: 70px;
    border: 2px dashed #ccc;
    background: none;
    float: left;
    position: relative;
    /* margin: 0 6px 6px 0; */
    margin: 15px;
    text-align: center;
    overflow: hidden;
}
.addDiv .add a {
    font-size: 60px;
    font-weight: bold;
    color: #ccc;
}
.addDiv .add .addUpDiv{
	position: absolute; left: 0px; top: 0px; display: block; overflow: hidden; background-color: rgb(0, 0, 0); opacity: 0; width: 80px; height: 80px;cursor: pointer;
}
.addDiv .add .addUpDiv input{
	cursor:pointer;width:1000px;height:1000px;position:absolute;bottom:0;right:0;font-size:200px;
}
.addicon{
	height:190px;
	text-align:center;
	background:#D8E5F0;
	margin-left:3%;
	position:relative;
	width:116px;
	mragin:3%;
	float:left;
	border:2px solid #C6C9C9;
}
.image2{
	line-height:190px;
	text-align:center;
}
.cgimagename{
	line-height:30px;
	text-align:center;
	font-size:15px;
	color:black;
}
.img2{
	width:80%;
	text-align:center;
}
.img{
	width:114px;
	height:160px;
}
.deletePhotog{
	position:absolute;right:0;top:0;
}
  </style>
  <body>
     <div id="" class="layui-layer-content">
		 <form class="layui-form" action="" lay-filter="example">
		  <div class="layui-form-item addtitle">
		    <label class="layui-form-label">名称</label>
		    <div class="layui-input-block" style="width:500px;">
		      <input id="name" name="name" lay-verify="required" autocomplete="off" placeholder="请输入名称" class="layui-input" type="text">
		    </div>
		  </div>
		  <div class="layui-form-item">
		    <label class="layui-form-label">别称</label>
		    <div class="layui-input-block"   style="width:500px;">
		      <input id="othername" name="othername" lay-verify="required" placeholder="请输入别称" autocomplete="off" class="layui-input" type="text">
		    </div>
		  </div>
		  <div class="layui-form-item">
		    <label class="layui-form-label">全称</label>
		    <div class="layui-input-block" style="width:500px;">
		      <input id="allname" name="allname" lay-verify="required" autocomplete="off" placeholder="请输入描述" class="layui-input" type="text">
		    </div>
		  </div>
		  <div class="layui-form-item layui-form-text">
		    <label class="layui-form-label">简述</label>
		    <div class="layui-input-block">
		      <textarea id="desc" name="desc" style="width:500px; placeholder="请输入内容" class="layui-textarea"></textarea>
		    </div>
		  </div>
		  <div class="layui-form-item">
		    <label class="layui-form-label">作品</label>
			<div id="workarea">

			</div>
		  </div>    
		</form>
     </div> 
     		<div id="btn1" class="layui-input-block">
		      <button class="layui-btn" onclick="changedata()">保存</button>
		    </div>
  </body>
<script src="layui/layui.all.js" charset="utf-8"></script>
<!-- 注意：如果你直接复制所有代码到本地，上述js路径需要改成你本地的 -->
<script>
var compid=window.parent.flag;
var editindex="";
var cgId = "<%=request.getAttribute("cgerId")%>";
var ischange=false;
	function changedata(){
		 if(ischange){
		var name=$("#name").val();
		var othername=$("#othername").val();
		var allname=$("#allname").val();
		var desc=$("#desc").val();
 	     $.ajax({
			url:"updateaggregation.do",
			type:"post",
			async: false,
			data:{
			"name":name,
			"othername":othername,
			"allname":allname,
			"desc":desc,
			"compid":compid
			},
			success:function(data){
				cleardata(compid);
			}
		});		 	
		}else{
			parent.layui.layer.msg("未进行任意修改");
		}
	}
$(function(){
	getdata();
	getImageInfo(compid);

});

layui.use(['form', 'layedit', 'laydate'], function(){
  var form = layui.form
  ,layer = layui.layer
  ,layedit = layui.layedit
  ,laydate = layui.laydate;
  
  //日期
  laydate.render({
    elem: '#date'
  });
  laydate.render({
    elem: '#date1'
  });

/*   form.on('submit(demo2)', function(data){
  if(ischange){ */
/*   		var name=$("#name").val();
		var othername=$("#othername").val();
		var allname=$("#allname").val();
		var desc=$("#desc").val();
 	     $.ajax({
			url:"updateaggregation.do",
			type:"post",
			async: false,
			data:{
			"name":name,
			"othername":othername,
			"allname":allname,
			"desc":desc,
			"compid":compid
			},
			success:function(data){
				cleardata(compid);
			}
		}); */
/* 		}else{
			parent.layui.layer.msg("未进行任意修改");
		}
	function cleardata(data){
		var compid=data;
		$.ajax({
			url:"editclear.do",
			type:"post",
			async: false,
			data:{
			"compid":data
			},
			success:function(data){
				addAG_WORKS2(compid);
			}
		});		
	}	
	function addAG_WORKS2(data00){
		$("#workarea").find('div[class="deletePhotog"]').each(function(){
		  var worksid= $(this).attr("id");
		  var worksname =$(this).parent().find('div[class="cgimagename"]').html();
		   $.ajax({
					url:"editaggregationImageInfo.do",
					type:"post",
					async: false,
					data:{
						  "worksname":worksname,
						  "worksid":worksid,
						  "compid":data00
							},
					success:function(data){
						ischange=false;
						parent.layer.closeAll();
						parent.location.reload();
					}
				});
	  });
	}	

  }); */

});
	function getdata(){
		$.ajax({
			url:"geteditaggregation.do",
			type:"post",
			data:{"compid":compid},
			dataType:"json",
			success:function(data){
				$("#name").val(data[0].compname);
				$("#othername").val(data[0].comothername);
				$("#allname").val(data[0].comallname);
				$("#desc").val(data[0].compdesc);
				}
			});
	}

 	function getIcon(){
		var code="<div class='layui-col-md2 addicon' style='border:2px solid #4F7A9E;'>"+
					"<div class='image2' id=''>"+
						"<img class='img2'  src='image/cgimage2.png'>"+	
					"</div>"+		   			
			   	  "</div>";
		$("#workarea").append(code);
		  	  
	 } 
  	function createImage(data1){
  			var code2="";
			code2+="<div class='addicon'>"+	
							"<div>"+	
								"<img class='img' src='selachievementimg.do?fileName="+data1.picname+"'>"+							
							"</div>"+
							"<div class='deletePhotog' id='"+data1.worksid+"' onclick='deleteImage(this)'>"+
							"<i class='iconfont' style='color:red;font-weight:bold;cursor: pointer;font-size: 16px;'>&#xe635;</i>"+
							"</div>"+
							"<div class='cgimagename'>"+data1.worksname+"</div>"+																										
			   	   "</div>";
			   	   $("#workarea").append(code2);		   	 
  	}
  	function getImageInfo(data){
  	var compid=data;
  		$.ajax({
			url:"getaggregationImageInfo.do",
			type:"post",
			data:{"compid":compid},
			dataType:"json",
			success:function(data){
			for(var i=0;i<data.length;i++){
					createImage(data[i])
					}
				addIcon();
					var title = $('#workarea');//the element I want to monitor 
					title.bind('DOMNodeInserted', function(e) { 
					ischange=true;
					}); 
					title.bind('DOMNodeRemoved', function(e) { 
					ischange=true;
					}); 			
				}
			});
  	}
  	function addIcon(){
		var code="<div id='icon' class='addicon'>"+
					"<div class='image2' id=''>"+
						"<img class='img2' onclick='openedit(this)'  src='image/cgimage2.png'>"+	
					"</div>"+		   			
			   	  "</div>";
			$("#workarea").append(code);

	} 
	function openedit(e){
			sondata=$(e.target).parent().parent().parent().attr("id");
	 	      editindex=layui.use(['form', 'layedit', 'laydate'], function(){
        		var form = layui.form
        		,layer = layui.layer
              		layer.open({
              		title: '添加'
	          		,type:2
	          		, content:'editaggregationmember.do?cgId='+cgId
	          		,area: ["650px", "500px;"],
          		}); 
        		});
	
	}
	function clear(){
		$("#icon").remove();	
	}

	function getImageInfo2(data){
  		var worksid=data;
  		if($('div').find("#"+worksid).length!=1){
  		$.ajax({
			url:"getInsertImageInfo.do",
			type:"post",
			data:{"worksid":worksid},
			dataType:"json",
			success:function(data){
			for(var i=0;i<data.length;i++){
					createImage2(data[i])
					}		
				}
			});	
			}else{
			 layui.layer.msg("作品已预加载合集中");
			}	
  	}
  	function createImage2(data1){
  			var code2="";
			code2+="<div class='addicon'>"+	
							"<div>"+	
								"<img class='img' src='selachievementimg.do?fileName="+data1.picname+"'>"+							
							"</div>"+
							"<div class='deletePhotog' id='"+data1.worksid+"' onclick='deleteImage(this)'>"+
							"<i class='iconfont' style='color:red;font-weight:bold;cursor: pointer;font-size: 16px;'>&#xe635;</i>"+
							"</div>"+
							"<div class='cgimagename'>"+data1.worksname+"</div>"+																										
			   	   "</div>";
			   	   $("#icon").before(code2);		   	 
  	}
  	
 		function deleteImage(e){
		var cgerid=$(e).attr("id");
		 	$(e).parent().remove();
	}
	
	$("input").bind('input propertychange',function(){
		ischange=true;
	 }); 
	 /* $("#workarea").bind('propertychange',function(){
	 	alert("2");
		ischange=true;
	 });  */
	function cleardata(data){
		var compid=data;
		$.ajax({
			url:"editclear.do",
			type:"post",
			async: false,
			data:{
			"compid":data
			},
			success:function(data){
				addAG_WORKS2(compid);
			}
		});		
	}	
	function addAG_WORKS2(data00){
		if($("#workarea").find('div[class="deletePhotog"]').length==0){
					parent.layer.closeAll();
					parent.location.reload();
		}else{
		$("#workarea").find('div[class="deletePhotog"]').each(function(){
		  var worksid= $(this).attr("id");
		  var worksname =$(this).parent().find('div[class="cgimagename"]').html();
		   $.ajax({
					url:"editaggregationImageInfo.do",
					type:"post",
					async: false,
					data:{
						  "worksname":worksname,
						  "worksid":worksid,
						  "compid":data00
							},
					success:function(data){
						/* alert(data); */
						ischange=false;
						parent.layer.closeAll();
						parent.location.reload();
					}
				});
	  });
	}	 
	}		  		
</script>

</body>
</html>