<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>
<%
String path = request.getContextPath();
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
%>

<!DOCTYPE HTML>
<html>
  <head>
    <base href="<%=basePath%>">
    
    <title>合集</title>
    
	<meta http-equiv="pragma" content="no-cache">
	<meta http-equiv="cache-control" content="no-cache">
	<meta http-equiv="expires" content="0">    
	<meta http-equiv="keywords" content="keyword1,keyword2,keyword3">
	<meta http-equiv="description" content="This is my page">
	<meta name="renderer" content="webkit">
	<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
	<link rel="stylesheet" href="css/common.css" type="text/css"></link>
	<link rel="stylesheet" href="layui/css/layui.css" type="text/css" media="all"></link>
	<link rel="stylesheet" href="layui/css/template.css" type="text/css"></link>
	<link rel="stylesheet" href="layui/css/admin.css" type="text/css"></link>
	<style type="text/css">
	@font-face {font-family: "iconfont";
	  src: url('iconfont.eot?t=1533979646640'); /* IE9*/
	  src: url('iconfont.eot?t=1533979646640#iefix') format('embedded-opentype'), /* IE6-IE8 */
	  url('data:application/x-font-woff;charset=utf-8;base64,') format('woff'),
	  url('iconfont.ttf?t=1533979646640') format('truetype'), /* chrome, firefox, opera, Safari, Android, iOS 4.2+*/
	  url('iconfont.svg?t=1533979646640#iconfont') format('svg'); /* iOS 4.1- */
	}
	
	.iconfont {
	  font-family:"iconfont" !important;
	  font-size:16px;
	  font-style:normal;
	  -webkit-font-smoothing: antialiased;
	  -moz-osx-font-smoothing: grayscale;
	}	
		.layui-layout-body,.maindiv{
			min-width: 860px !important;
		}
		.cmdlist-container{
			max-width: 300px;
			max-height: 360px;
			border: 1px #eee solid !important;
		}
		.cmdlist-container img{
			min-width: 120px;
			min-height: 120px;
			max-width: 300px;
			max-height: 300px;
		}
		.searchdiv{
			height: 60px;
			background:#fff;
			z-index:1;
		}
		.searchdiv .layui-inline{
			margin-top: 10px;
		}
		.searchdiv .layui-inline .layui-form-label{
			width: auto !important;
		}
		#photoglist{
			min-height: 300px !important;
		}
		#photoglist .cmdlist-container{
			cursor: pointer;
		}
		
		#photoglist,#footpage{
			margin-left:5px;
			margin-right:5px;
		}
		
		/* 阴影 */
		.shadow{
			-webkit-box-shadow:0px 0px 3px #a0a0a0;
	    	-moz-box-shadow:0px 0px 3px #a0a0a0;
	    	box-shadow:0px 0px 3px #a0a0a0;
			/* 投影数字顺序依次是：右边 下边 投影模糊大小 */
			/* 内阴影数字顺序依次是：左边 上边 阴影模糊大小 */
		}
		.top3{
			float:right;
			margin-left:3%;
			margin-right:3%;
		}
		.top2{
			float:right;
		}
		.deletePhotog{
			position:absolute;right:0;top:0;
		}
		.deletePhotog2{
			position:absolute;right:120px;top:40px;z-index:999;
		}
		.deletePhotog3{
			position:absolute;right:40px;top:40px;z-index:999;
		}			
		.aggregationtitle{
			margin-left:2%;
			height:30px;
			line-height:30px;
		}
		.addicon{
			height:260px;
			text-align:center;
			background:#D8E5F0;
			margin-left:3%;
			position:relative;
			width:230px;
			mragin:3%;
			float:left;
			border:2px solid #C6C9C9;
		}
		.image2{
			height:160px;
			line-height:160px;
			margin-top: 50px;
		}
		.img{
			width:130px;
			height:130px;
		}
		.cgimagename{
			line-height:30px;
			text-align:center;
			font-size:15px;
			color:black;
		}
		
		
		.addicon img{
			width: 230px !important;
			height: 230px !important;
		}
		.addicon2{
			height:260px;
			text-align:center;
			background:#D8E5F0;
			margin-left:3%;
			position:relative;
			width:230px;
			mragin:3%;
			float:left;
			border:2px solid #C6C9C9;
		}
		.img2{
			width: 130px !important;
			height: 130px !important;
		}
	</style>
	
	<script type="text/javascript" src="js/jquery-1.9.1.js"></script>
	<script type="text/javascript" src="js/jscommon.js"></script>
	<script type="text/javascript" src="js/jquery.nicescroll.js"></script>
	<!-- 让IE8/9支持媒体查询，从而兼容栅格 -->
	<!--[if lt IE 9]>
	  <script src="js/html5.min.js"></script>
	  <script src="js/respond.min.js"></script>
	<![endif]-->
  </head>
  
  <body class="layui-layout-body" style="background-color:white">
    <div class="maindiv" id="maindiv" style="height:800px;overflow-y:auto">
    	<div class="searchdiv">
    	    <div class="layui-inline">
		    </div>
      		<div class="layui-inline top3">
		      <div class="layui-input-inline">
		        <button class="layui-btn" onclick="newaggregation()" style="margin-top: 1px !important;">新增合集</button>
		      </div>
		    </div>
      		<div class="layui-inline top2">
		      <div class="layui-input-inline">
		        <button class="layui-btn" onclick="searchaggregation()" style="margin-top: 1px !important;">检索</button>
		      </div>
		    </div>		    			    
      		<div class="layui-inline top2">
		      <label class="layui-form-label">名称</label>
		      <div class="layui-input-inline">
		        <input type="text" id="serchrgName" autocomplete="off" class="layui-input">
		      </div>
		    </div>
		    
		    <hr class="layui-bg-grey">
	    </div>
	    <div id="centerdiv">
<!-- 		  <div class="layui-row layui-col-md12">
		    <div class="layui-col-md12">
		    	<div class="layui-col-md2 aggregationtitle">礼乐集</div>
				<button class="layui-btn deletePhotog" onclick="editaggregation()">编辑</button>
		    </div>
		    <hr class="layui-bg-grey">
		    <div class="layui-col-md12" id="test1">
		    </div>			    	    
		  </div> -->
	    </div>
    </div>
  </body>
  <script type="text/javascript" src="layui/layui.all.js"></script>
  <script type="text/javascript">
   var cgId = "<%=request.getAttribute("cgerId")%>";
   var sondata="";
  $(function(){
  	getdata();  	
  });
  	var flag="";
  	function createImage(data0,data1,data2){
  			var code2="";
  		/* 	alert(data1.worksid+":"+data1.worksname+":"+data1.isimpt); */
  		 	if(data2!=5){ 
			code2+="<div class='addicon'>"+	
							"<div data-id='"+data1.worksid+"' data-name='"+data1.worksname+"' data-isimpt='"+data1.isimpt+"' onclick='showdetail(this)'>"+	
								"<img class='img' src='selachievementimg.do?fileName="+data1.picname+"'>"+							
							"</div>"+
							"<div class='deletePhotog' id='"+data1.worksid+"' data-src='"+data0+"' onclick='deleteImage(this)'>"+
							"<i class='iconfont' style='color:red;font-weight:bold;cursor: pointer;font-size: 16px;'>&#xe635;</i>"+
							"</div>"+
							"<div class='cgimagename'>"+data1.worksname+"</div>"+																										
			"</div>";
		 	}else{
			code2+="<div class='addicon'>"+	
							"<div data-id='"+data1.worksid+"' data-name='"+data1.worksname+"' data-isimpt='"+data1.isimpt+"' onclick='showdetail(this)'>"+	
								"<img class='img' src='selachievementimg.do?fileName="+data1.picname+"'>"+							
							"</div>"+
							"<div class='cgimagename'>"+data1.worksname+"</div>"+																										
			"</div>";			
			} 
			   $("#"+data0).append(code2);	
				 
  	}
  	
  	function getImageInfo(data,data2){
  	var compid=data;
  		$.ajax({
			url:"getaggregationImageInfo.do",
			type:"post",
			data:{"compid":compid},
			dataType:"json",
			success:function(data){
			for(var i=0;i<data.length;i++){
					createImage(compid,data[i],data2)
					}
				addIcon(compid);			
				}
			});
  	}
  	function getdata(){
  		$.ajax({
			url:"getaggregation.do",
			type:"post",
			data:{"cgId":cgId},
			dataType:"json",
			success:function(data){
			var code="";
			for(var i=0;i<data.length;i++){
				if(data[i].comstatus!=5){
				 code+="<div class='layui-row layui-col-md12' style='margin-bottom:1%'>"+
				    "<div class='layui-col-md12'>"+
				    	"<div class='layui-col-md2 aggregationtitle'>"+data[i].compname+"</div>"+
						"<button class='layui-btn deletePhotog2' value='"+data[i].compid+"'  onclick='editaggregation(this)'>编辑</button>"+
						"<button class='layui-btn deletePhotog3' value='"+data[i].compid+"'  onclick='deleteaggregation(this)'>删除</button>"+
				    "</div>"+
				    "<hr class='layui-bg-grey'>"+
				    "<div class='layui-col-md12' id='"+data[i].compid+"'>"+
				    "</div>"+			    	    
				  "</div>"
				}else if(data[i].comstatus=='5'){
				 code+="<div class='layui-row layui-col-md12' style='margin-bottom:1%'>"+
				    "<div class='layui-col-md12'>"+
				    	"<div class='layui-col-md2 aggregationtitle'>"+data[i].compname+"</div>"+
						"<button class='layui-btn deletePhotog2' style='background:grey' value='"+data[i].compid+"' disabled=''  onclick='editaggregation(this)'>编辑</button>"+
						"<button class='layui-btn deletePhotog3' style='background:grey' value='"+data[i].compid+"' disabled=''  onclick='deleteaggregation(this)'>删除</button>"+				   
				    "</div>"+
				    "<hr class='layui-bg-grey'>"+
				    "<div class='layui-col-md12' id='"+data[i].compid+"'>"+
				    "</div>"+			    	    
				  "</div>"				
				}
			}
			$("#centerdiv").append(code);
			 for(var i=0;i<data.length;i++){
				getImageInfo(data[i].compid,data[i].comstatus);
			}				 	 
			}
		});
		
  	} 
  	function openadd(e){		
			sondata=$(e).parent().parent().parent().attr("id");
	 	       layui.use(['form', 'layedit', 'laydate'], function(){
        			var form = layui.form
        			,layer = layui.layer
              		layer.open({
              		title: '添加成员'
	          		,type:2
	          		, content:'addaggregationmember.do?cgId='+cgId
	          		,area: ["650px", "500px;"],
          			 }); 
        		});
  	}

  	function addIcon(data){
		var code="<div class='addicon2'>"+
					"<div class='image2' id=''>"+
						"<img id='img2' class='img2' onclick='openadd(this)'  src='image/cgimage2.png'>"+	
					"</div>"+		   			
			   	  "</div>";
			$("#"+data).append(code);
	}
	//右上角添加合集入口
	function newaggregation(){
			layui.use(['form', 'layedit', 'laydate'], function(){
        			var form = layui.form
        			,layer = layui.layer
              		layer.open({
              			title: '新增合集'
	          			,type:2
	          			, content:'newaggregation.do?cgId='+cgId
	          			,area: ["980px", "650px"],
          			}); 
        	});		
	}
	//右测编辑合集入口
	function editaggregation(e){
			var compid=$(e).val();
			flag=compid;
			layui.use(['form', 'layedit', 'laydate'], function(){
        			var form = layui.form
        			,layer = layui.layer
              		layer.open({
              			title: '编辑'
	          			,type:2
	          			, content:'editaggregation.do?cgId='+cgId
	          			,area: ["880px", "650px"],
          			}); 
        	});		
	}
	//删除合集
		function deleteaggregation(e){
			var compid=$(e).val();
  		$.ajax({
			url:"deleteaggregationInfo.do",
			type:"post",
			data:{"compid":compid},
			success:function(data){
				layui.layer.msg(data);
				$(e).parent().parent().remove();		
				}
			});			
		}
		//搜索合集
	function searchaggregation(){
		$("#centerdiv").html("");
		var name=$("#serchrgName").val();
		  		$.ajax({
			url:"searchaggregation.do",
			type:"post",
			data:{"name":name,"cgId":cgId},
			dataType:"json",
			success:function(data){
			var code="";
			for(var i=0;i<data.length;i++){
				if(data[i].comstatus!='5'){
				 code+="<div class='layui-row layui-col-md12' style='margin-bottom:1%'>"+
				    "<div class='layui-col-md12'>"+
				    	"<div class='layui-col-md2 aggregationtitle'>"+data[i].compname+"</div>"+
						"<button class='layui-btn deletePhotog2' value='"+data[i].compid+"'  onclick='editaggregation(this)'>编辑</button>"+
				    "</div>"+
				    "<hr class='layui-bg-grey'>"+
				    "<div class='layui-col-md12' id='"+data[i].compid+"'>"+
				    "</div>"+			    	    
				  "</div>"
				}else if(data[i].comstatus=='5'){
				 code+="<div class='layui-row layui-col-md12' style='margin-bottom:1%'>"+
				    "<div class='layui-col-md12'>"+
				    	"<div class='layui-col-md2 aggregationtitle'>"+data[i].compname+"</div>"+
						"<button class='layui-btn deletePhotog2' style='background:grey' value='"+data[i].compid+"' disabled=''  onclick='editaggregation(this)'>编辑</button>"+
				    "</div>"+
				    "<hr class='layui-bg-grey'>"+
				    "<div class='layui-col-md12' id='"+data[i].compid+"'>"+
				    "</div>"+			    	    
				  "</div>"				
				}
			}
			$("#centerdiv").append(code);
			
			 for(var i=0;i<data.length;i++){
				getImageInfo(data[i].compid,data[i].comstatus);
			}	 
			}
		});
	}
		//删除某个合集中的某张图片
		function deleteImage(e){
		var worksid=$(e).attr("id");
		var compid=$(e).attr("data-src");
		    layer.confirm('确定删除吗？', function(index) { 
 			 $.ajax({
				url:"deleteaggregationmember.do",
				type:"post",
				data:{"worksid":worksid,"compid":compid},
				success:function(data){
					layui.layer.msg('<span style="font-size:20px;color:white">'+data+'</span>');
				}
			});
		 	$(e).parent().remove();
			layer.close(index);
		 }); 
	}
//点击合集中图片展示详情
	function showdetail(e){
		var workId=$(e).attr("data-id");
		var workName=$(e).attr("data-name");
		var isImpt=$(e).attr("data-isimpt");
        layui.use(['layer','form'], function(){
        	layer = layui.layer;
	        window.parent.window.parent.layer.open({
		        type: 2 //此处以iframe举例
		        ,title: workName
		        ,area: ['1000px', '700px']
		        ,shade: 0.3
		        ,maxmin: true
		        ,offset: 'auto'
		        ,content: 'worksDetailCPG.do?workId='+workId+'&cgerId='+cgId+'&isImpt='+isImpt
		        ,zIndex: layer.zIndex //重点1 
		        ,success: function(layero){	
		        }
	        });
        });		
	}
  </script>
</html>
