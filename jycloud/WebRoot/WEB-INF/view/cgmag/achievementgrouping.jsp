...
<!-- 		<div class="option layui-col-md5" >
  ...
  <script type="text/javascript">
			...
			success:function(data){
				...
				$(".image2").click(function(e){
              		...
              		layer.open({
              			title: '添加成员'
	          			,type:2
	          			, content:'addachievementmember.do'
	          			,area: ["630px", "480px;"],
          			  }); 
        			  });
				});
	...
	function searchAchievementaginfo(){
		...
		$.ajax({
			url:"searchAchievementaginfo.do",
			type:"post",
			dataType:"json",
			data:{"data":data},
			success:function(data){
	...
	for(var i=0;i<data.length;i++){
		...
		"</div>";	
			...
			code+="<div class='option layui-col-md5' id='"+data[i].agid+"'>"+
				"<div class='deletePhotog' onclick='deleteModule(this)' title='删除成就' >"+
		...
		code+="<div class='option layui-col-md5'>"+	
		...
		$("#add").click(function(){
              		...
              		layer.open({
              			title: '添加'
	          			,type:2
	          			, content:'addagachievement.do'
	          			,area: ["650px", "360px"],
          			  }); 
        			  });
		});
	}
	function createImage(agid,data0,data){
		var code2="";
		if(data.agstate==5){
			...
			code2+="<div class='addicon'>"+	
							"<div>"+	
								"<img class='img' src='selachievementimg.do?fileName="+data0.filename+"'>"+							
							"</div>"+
							"<div class='deletePhotog' title='删除成员' id='"+data0.cgerid+"'data-name='"+agid+"' onclick='deleteImage(this)'>"+
							"<i class='iconfont' style='color:red;cursor:pointer;font-weight:bold;font-size: 16px;'>&#xe635;</i>"+
							"</div>"+
							"<div class='cgimagename'>"+data0.cgername+"</div>"+																										
			   	   "</div>";		
		}
	...
	function deleteImage(e){
		...
		var agid=$(e).attr("data-name");
		   layer.confirm('确定删除吗？', function(index) {
	 		 $.ajax({
				url:"deletemember.do",
				type:"post",
				data:{"cgerid":cgerid,"agid":agid},
				success:function(data){
	...
	function deleteModule(e){
		var agid=$(e).parent().attr("id");
		  layer.confirm('确定删除吗？', function(index) {
			 $.ajax({
				url:"deleteachievement.do",
				type:"post",
				data:{"agid":agid},
				success:function(data){
					layui.layer.msg('<span style="font-size:20px;color:white">'+data+'</span>');
