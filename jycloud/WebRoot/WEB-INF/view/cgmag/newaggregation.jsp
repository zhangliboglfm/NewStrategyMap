...
/* 	parent.layer.closeAll();
	...
	$("#workarea").find('div[class="deletePhotog"]').each(function(){
	  ...
	  var worksname =$(this).parent().find('div[class="cgimagename"]').html();
	   $.ajax({
				url:"newaggregationImageInfo.do",
				type:"post",
				async: false,
				data:{
					  "worksname":worksname,
  	...
  	function createImage(data1,count){
  			var code2="";
			code2+="<div class='addicon'>"+	
			   	   ...
			   	   $("#workarea").append(code2);
			   	    if($("#workarea").find('div[class="addicon"]').length==count){
			   	    	addIcon();	
			   	    };		  		   	 
  	}
  	
  	function getImageInfo(data,count){
  	var worksid=data;
  		$.ajax({
		...
		var code="<div class='addicon'>"+
			...
			$("#workarea").append(code);
              		...
              		layer.open({
              			title: '添加'
	          			,type:2
	          			, content:'newaggregationmember.do?cgId='+cgId
	          			,area: ["650px", "500px;"],
          			  }); 
        			  });
				});	
