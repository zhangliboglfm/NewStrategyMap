		...
		<div class="rightpath">
			<div id="workinfordiv" class="workinfordiv">
				<ul>
					...
					<li><div id="yearnum" onclick="changeDynasty()" class="eachinfor"><div class="lab">年代：</div><input id="Years" name="cgworkinfor" value="" type="text" readonly="readonly"/></div></li>
					...
					<li style="float:right;margin-top:10px;margin-right:10px;width: 80px"><div class="saveconfig" onclick="saveinfors()">保&nbsp;&nbsp;存</div></li>
				</ul>
			</div>
...
<script type="text/javascript">
	...
	function showworksList(data,data2,data3){
		...
		for ( var i = 0; i < data.length; i++) {
			...
			var status=data[i].AUDIT_STATUS
			html+="	<div class='listcont-child shadow' id='"+workid+"' data-cgid='"+cgid+"' data-status='"+status+"' onclick='worksBaseInfo(this)'> "+
		...
		$("#listcont").html(html);
		for ( var i = 0; i < data.length; i++) {
		var workid2=data[i].WORKS_ID;
		var status2=data[i].AUDIT_STATUS
		if (workid2==data2) {
			$(".listcont-text[id="+workid2+"]").addClass("classfiy");
			$(".listcont-text[id="+workid2+"]").siblings().removeClass("classfiy");
			//获取作品相关信息
			workinfors(workid2,cgid,status2);
		}
		}
	}
	//得到作品图片信息
	...
	function worksBaseInfo(e){
		...
		workinfors(workid,cgid,status);
	}
	function workinfors(data1,data2,data3){
		$.ajax({
			...
			success:function(data){
				...
				if(data3==5){
					...
					$(".saveconfig").css("display","none");
					$("#yearnum").removeAttr("onclick");
					}else{
					$(".eachinfor input").addClass("inputboder");
					$(".eachinfor input").removeAttr("readonly");
					$(".saveconfig").css("display","block");
					$("#yearnum").attr("onclick","changeDynasty();");
				}
				if(data3==5){
				$("#ifrcgworkImg").attr("src","cgworkseditorCPG.do?workid="+data1+"&cgid="+data2);				
				}else{
				$("#ifrcgworkImg").attr("src","cgworkseditor.do?workid="+data1+"&cgid="+data2);				
				}
			}
		});
	}
...
/* 		iseditor=false;
		...
		$(".eachinfor input").removeClass("inputboder"); */
		if(ischange){
		flushdata(workId,cgerId);
		}else{
		layui.layer.msg('<span style="font-size:20px;color:white">未进行任意修改操作</span>');
		}
