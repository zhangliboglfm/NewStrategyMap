...
<html>
    	...
    	<div class="content">
    		...
    		<div class="content2" id="ct3" style="width:100%;height:13%;">
    			<div class="content3" style="width:100%;height:100%">
    				...
    				<div class="content4 buff" style="width:40%;text-align: center;"><button class="layui-btn layui-btn-sm layui-btn-normal" onclick="regresses()">回退</button></div>
    			</div>
    		</div>
    	</div>
    </div>
  ...
  <script type="text/javascript">
	...
	function seeedit(id){
		...
		w3+="</div>"; 
		w3+="<div class='sedit'>";
		w3+="<div class='state5'><button class='layui-btn layui-btn-sm layui-btn-normal buttonedit' id='"+id+"' onclick='regressesPic(this)'>回退</button></div>";
...
/* 		w3+="<div><button class='layui-btn layui-btn-sm layui-btn-normal ' id='"+id+"' onclick='screenpics(this)'>保存并筛选</button></div>"; */
	...
	function see(obj){
		...
		layer.open({
	        ...
	        ,content: 'reviewsampleaudit.do?id='+id
	        ,btn: ['回退', '取消']
    		...
    		//按钮【按钮一】的回调
    			regressesPic2(data[0].SAMP_PIC_ID);
    			layer.close(index);
		...
		w+="<div class='state5'><button class='layui-btn layui-btn-sm layui-btn-normal buttonedit' onclick='regressesPic2(this)' id='"+data[0].SAMP_PIC_ID+"'>回退</button></div>";
		w+="</div>";
		w+="</div>";
		$("#statepic").html("");
		$("#statepic").html(w);
	...
	function rejectReason(id){
		...
		layer.open({
    		...
    		//按钮【确认】的回调
    			...
    			enter2regersses(content,reId,id);
    			layer.close(index);
  			},btn2: function(index, layero){
    		//按钮【取消】的回调
	...
	//回退原因弹出框确认事件
	function enter2regersses(content,reId,id){
		//var content=$("#reasonPut").val().trim();
		/* var reId="";
		$("#reasonul input[name=regressescheck]:checked").each(function(){
		...
		if(reId.length>0){
			addRegresses(id,"胶卷","3","无","jyyr",reId);
		}
		if(content!=""){
			reId+=",0010";
			addReason("0010","回退",content,content,"0");
		}
		$("#reason").hide();
		$("#background").hide();
	};
