		...
		$.ajax({
			 ...
			 success:function(data){
			 	...
			 	for(var i = 0;i<data.photogName.length;i++){
			 		pgId=data.photogId[0];
			 		if (i == 0) {
						photogCode+="<li data-id='"+data.photogId[i]+"' class='pgCate layui-this'>"+data.photogName[i]+"</li>";
					} else {
						photogCode+="<li data-id='"+data.photogId[i]+"' class='pgCate'>"+data.photogName[i]+"</li>";
					}
			 	}
			 	$("#layuiUl").html("");
	...
	function loadworkslist(currPage,pageSize){
		...
		ajaxRequest(url, parameter, function(data){
			...
			for(var i=0;i<dataList.length;i++){
	    		...
	    		html+='		<a><img src="getArtImage.do?imgUrl='+filename+'&imgFlag=3"></img></a>';
