...
<link rel="stylesheet" href="css/relateArticle.css" type="text/css"></link>
...
<body>
	<div class="layui-tab layui-tab-brief tabdiv" lay-filter="test">
		<ul class="layui-tab-title">
			...
			<li id="artUpload" lay-id="upload">文章录入</li>
		</ul>
...
<script type="text/javascript">
	...
	//数据展示
	function showArticleInfo(curr){
		$.ajax({
			url:"showArticleInfo.do",
			type:"post",
			dataType:"json",
			data:{
				photogId : photogId,
			...
			success:function(data){
				...
				for(var a=0;a<data.articleTitle.length;a++){
					htmlCode+="<div class='layui-col-md2 layui-col-sm4'>"+
								...
								"<a><img src='getArtCover.do?imgUrl="+data.articleCover[a]+"&photogId="+photogId+"'></a>"+
	...
	function showArtInfo(e) {
        ...
        layui.use(['layer','form'], function(){
	        ...
	        window.parent.window.parent.layer.open({
		        type: 2 //此处以iframe举例
		        ,title: photogName+'的相关文章'
		        //,area: [($(window).width())*2/3+'px', ($(window).height()-40)+'px']
		        ,area: ['1000px', '700px']
		        ,shade: 0
		        ,maxmin: true
		        ,offset: 'auto'
		        ,content: 'articleDetail.do?photogId='+photogId+'&articleId='+articleId
	...
	function uplaodFile(){
		...
		var url = "uploadArtZip.do?name="+filename+"&size="+filesize;
		var xhr;
