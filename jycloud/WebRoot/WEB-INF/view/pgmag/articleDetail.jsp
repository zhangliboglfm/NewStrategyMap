			 	...
			 	for(var i = 0;i<data.worksName.length;i++){
			 		...
			 		photogCode+="<div class='layui-col-md2 layui-col-sm4 layui-col-sm46' data-id='"+data.worksId[i]+"'>"+
						        ...
						        "<div class='cmdlist-container cmdlist-container2'>"+
						              "<img class='speacilImg' src='getArtImage.do?imgUrl="+data.fileName[i]+"&imgFlag=3'>"+
						              "<div class='cmdlist-text'>"+
			...
			if (strArr.length==0) {
				...
				layui.use(['layer','form'], function(){
			        ...
			        window.parent.layer.open({
				        ...
				        ,maxmin: true
				        ,offset: 'auto'
				        ,content: 'upLongPic.do?bigTage=article&flagId='+articleId
	        	...
	        	if (data.flag[0]!=1) {
					...
					if ("artTitle"==coverOrfile) {
						...
						if ("wordReading"==fileType) {
							...
							$("#myiframe2").attr("src","getArtImage.do?imgUrl="+data.imgPath[0]+"&imgFlag=1");
						}
					}
				}
	       	}
					...
					success:function(data){
						...
						if(data.articlePic[0]!="" && data.articlePic[0]!="null" && data.articlePic[0]!="undefined"){
							...
							for (var a=0;a<strArr.length;a++) {
								str+="<img class='showImg' src='getArtImage.do?imgUrl="+strArr[a]+"&imgFlag=1'></img>";
							}
							$("#showImgD").html("");
							$("#showImgD").html(str);
