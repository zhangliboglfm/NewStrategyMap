...
function download(){
	...
	window.open("downachieve.do?sel="+cur_sel+"&artname="+artname+"&artpic="+artpic+"&arttitle="+encodeURI(encodeURI(arttitle)));
}
/**
 * 上传
 */
		...
		if (artpic==null||artpic=="null"||artpic=="") {
			...
			layui.use(['layer','form'], function(){
		        ...
		        window.parent.layer.open({
			        ...
			        ,maxmin: true
			        ,offset: 'auto'
			        ,content: 'upLongPicCg.do?bigTage=achievement&pgorcg=cg&flagId='+aId+'&cgId='+cgId
...
function uploadWord(){
	...
	var url = "uploadcgachieve.do?name="+encodeURI(encodeURI(filename))+"&size="+filesize+"&cgId="+cgId+"&aId="+aId;
	var xhr;
...
function uploadPic(){
	...
	var filename = file.name;
	var filesize = file.size;
	var url = "uploadcgachievepic.do?name="+encodeURI(encodeURI(filename))+"&size="+filesize+"&cgId="+cgId+"&aId="+aId;
	var xhr;
	if (window.XMLHttpRequest){
		// code for IE7+, Firefox, Chrome, Opera, Safari
		xhr=new XMLHttpRequest();
