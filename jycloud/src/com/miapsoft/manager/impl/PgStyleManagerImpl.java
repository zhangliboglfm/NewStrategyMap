...
@Service
public class PgStyleManagerImpl extends AbstractManager implements PgStyleManager {
	...
	public JSONObject getPgStyleWord(String pId) {
		...
		String sql = "SELECT B.ARTICLE_ID,B.ARTICLE_TITLE,B.ARTICLE_DOC,B.ARTICLE_PIC,B.ARTICLE_URL,B.ARTICLE_TITLE FROM TB_CDE_PHOTOG_ARTICLE_RELAT A LEFT JOIN TB_PHOTOG_ARTICLE_BASE_INFO B ON A.ARTICLE_ID = B.ARTICLE_ID "+
					 "WHERE A.PHOTOG_ID=? AND B.ARTICLE_TYPE='ST'";
	...
	public String insertIntoNewArticle(JSONObject obj, String pId) {
		...
		String idsql = "select MAX(right(ARTICLE_ID, 8)) from TB_PHOTOG_ARTICLE_BASE_INFO WHERE ARTICLE_TYPE='ST'";
	...
	public String insertIntoNewStyleWithPic(String picname, String pId) {
		...
		if (list.size()!=0) {
			pgName=list.get(0).get("PHOTOG_NAME")+"风格";
		}
		String aId = IdGenerateUtil.getArticleId("ST", maxidint+1);
