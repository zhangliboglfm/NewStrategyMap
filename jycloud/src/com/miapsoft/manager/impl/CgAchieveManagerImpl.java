...
@Service
public class CgAchieveManagerImpl extends AbstractManager implements CgAchieveManager {
	...
	public JSONObject getCgAchieveWord(String cgId) {
		...
		String sql = "SELECT B.ARTICLE_ID,B.ARTICLE_TITLE,B.ARTICLE_DOC,B.ARTICLE_PIC,B.ARTICLE_URL,B.ARTICLE_TITLE "+
						...
						"WHERE A.CGER_ID=? AND B.ARTICLE_TYPE='AC'";
	...
	public String insertIntoNewArticle(JSONObject obj, String cgId) {
		...
		String idsql = "select MAX(right(ARTICLE_ID, 8)) from TB_CG_ARTICLE_BASE_INFO WHERE ARTICLE_TYPE='AC'";
	...
	public String insertIntoNewAchievementWithPic(String picname, String cgId) {
		...
		String pgName="无名氏";
		String pgnSql="SELECT CGER_NAME FROM TB_CG_BASE_INFO WHERE PHOTOG_ID='"+cgId+"' ";
		List<Map<String, Object>> list = this.getJdbcTemplate().queryForList(pgnSql);
		if (list.size()!=0) {
			pgName=list.get(0).get("CGER_NAME")+"成就";
		}
		String aId = IdGenerateUtil.getArticleId("AC", maxidint+1);
